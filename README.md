...
短信入口：选择最近1–365天及可选精确发送方，逐次授权读取最新30条收件短信，勾选后预览实际上传内容，再由 DeepSeek 提取自定义字段并筛选疑似垃圾短信。识别出的验证码及超过2000字的短信不上传；其他所选正文在确认后发送，不承诺自动脱敏。结果为带来源、证据的 CSV；疑似垃圾可单独筛选并跳转原短信应用处理。只声明 `READ_SMS`，不成为默认短信应用、不发送或删除短信。首次使用需允许系统短信读取权限。真机说明见 [短信验收](docs/SMS-VALIDATION.md)。
